...
antho.simul$yhat <- predict(ant.fm0, newdata = anthocyanins)
...
anthocyanins_pred2 <- presiones.l_pred2[which(presiones.l_pred2$compound %in% c("Delphinidin.3.O.sambubioside.5.O.glucoside",	
                                                              ...
                                                              "Delphinidin.3.O.sambubioside",	"Delphinidin.3.O.glucoside",	
                                                              "Cyanidin.3.O.sambubioside..Cyanidin.3.O.glucoside")),]
antho.simul<-expand.grid(compound=levels(anthocyanins$compound),
                        Temp=c(20,4),
                        sweetener=levels(anthocyanins$sweetener),
                        processing=levels(anthocyanins$processing),
                        tiempo=seq(0,90,length = 50),
                        grouping=levels(anthocyanins$grouping))

levels(anthocyanins$Temp)<-c(20,4,20,4,20,4)
anthocyanins$Temp<-as.numeric(as.character(presiones.l_pred2$Temp))
anthocyanins$grouping<-with(anthocyanins,sweetener:processing:factor(Temp))

ant.simul2<-expand.grid(tiempo=seq(0,90,length=50),
                        Temp = c(20,4),
                       compound=levels(factor(anthocyanins$compound)),
                       grouping=levels(factor(anthocyanins$grouping))
)
...
#ant.simul2$Temp<-as.numeric(with(ant.simul2,substr(grouping,6,8)))
...
antho.simul$yhat <- predict(ant.fm3, newdata = antho.simul)
...
texreg(list.of.compound.fit,single.row=T,ci.force=T,
      groups=list("C0 [mg/100mL]"=1:4,"Cinf [mg/100mL]"=5:8,"ln(k) [1/min]"=9:12,"Ea [KJ/Mol/K]"=13),
      custom.gof.rows=list("R2adj"=list.of.r2adj),
      file="anthocyanins.html",
      caption.above=T,
...
ant.pred2$prediction<-predict(ant.fm3_pred2,newdata = ant.pred2)

ggplot(data=ant.pred2,aes(x=tiempo,y=prediction,col=grouping)) +
  facet_wrap(compound~.,scales="free")+
  geom_point(data=anthocyanins_pred2,aes(x=tiempo,y=concentration,col=grouping))+
  xlab("Storage Time [Days]")+ylab("Concentration [mg/100mL]")+
  geom_line()
...
ggplot(ant.pred2, 
       aes(x = tiempo, y = log(prediction), group =grouping,color=factor(Temp))) +
  facet_wrap(compound~.,scales="free")+
  geom_point()+ geom_smooth(method="lm",se = F)+
  xlab("Storage Time [Days]")+ylab("Concentration mg/100mL)")
...
ggplot(data = subset(ant.pred2,grouping == "ST:1:20"), aes(x=tiempo, y = prediction, col = Temp))+
       facet_wrap(compound~.,scales="free")+
       geom_point()+ geom_line(data=subset(ant.pred,grouping == "ST:1:4"), aes(x=tiempo, y=concentration, col = Temp))+
  ggtitle("ST:P 20º modelled | 4º experimental") + xlab("Storage Time [Days]")+ylab("Concentration [mg/100mL]")

# ST:2
ggplot(data = ant.pred2 %>% filter(grouping == "ST:2:4"), aes(x=tiempo, y = prediction, col = Temp))+
  facet_wrap(compound~.,scales="free")+
...
# ST:P
ggplot(data = ant.pred2 %>% filter(grouping == "ST:P:20"), aes(x=tiempo, y = prediction, col = Temp))+
  facet_wrap(compound~.,scales="free")+
...
ggplot(data = ant.pred2 %>% filter(grouping == "SU:1:4"), aes(x=tiempo, y = prediction, col = Temp))+
  facet_wrap(compound~.,scales="free")+
...
ggplot(data = ant.pred2 %>% filter(grouping == "SU:2:20"), aes(x=tiempo, y =prediction, col = Temp))+
  facet_wrap(compound~.,scales="free")+
...
ggplot(data = ant.pred2 %>% filter(grouping == "SU:P:4"), aes(x=tiempo, y = prediction, col = Temp))+
  facet_wrap(compound~.,scales="free")+
